...
| 2-3 | **Resolved 2026-08-24, later same day:** Full `brew audit`/`brew install`/`brew services start` were initially blocked on this dev machine by outdated Xcode Command Line Tools. User updated CLT; re-ran for real. `brew audit --strict --online` surfaced 2 real findings (redundant `version` line, `assert_predicate :exist?` → `assert_path_exists`) — fixed and pushed (`homebrew-tap@64b52b2`), re-audited clean. `brew services start saki` initially failed (`error 1`) — root cause: a leftover lazy CLI daemon from earlier in the session held port 8788, exactly the documented coexistence scenario; `saki backend stop` + `brew services restart saki` succeeded, health check returned `{"ok":true}`, `saki backend status` correctly reported healthy via the state-truthy branch. Machine cleaned up afterward (stop, uninstall, untap) | `brew audit` clean output; `brew services list` → `started`; `curl /api/health` → `{"ok":true,...}`; `saki backend status` → `backend healthy (pid <real>)` |

**Post-implementation `/saki-builder:reviewer` pass (2026-08-24) — 0 blockers, 5 warnings, all resolved:**

| # | Finding | Resolution |
|---|---|---|
| W1 | The comment at step 4's code claimed a service-managed backend "never goes through `ensureDaemon()`/`writeState()`, so [there's] no state file" — factually wrong for the common case. Verified directly: `backend/cmd/server/main.go:184` calls `writeDaemonState()` **unconditionally** on every startup (same `daemonStatePath()` logic, keyed on uid + `TMPDIR`), so a same-user brew/systemd service already gets picked up by the existing `state`-truthy branch. The null-state branch only matters when uid/`TMPDIR` diverge (`sudo brew services`, a systemd unit under another user). Code fix from step 4/5 is still correct and worth keeping (a real, if narrower, scenario) — only the rationale/example was wrong. | Corrected the comment in `src/commands/backend.ts` to name the real condition (uid/TMPDIR divergence) instead of the false premise; reworded the human-output message for the `pid:null, healthy:true` case to `backend healthy (service-managed, no local state file)`. |
| W2 | Follows from W1: since the Go binary writes a real state file for a same-user service, `saki backend stop` can SIGTERM/SIGKILL a brew-managed process too — self-heals via the formula's `keep_alive: true`, but surprising. README's "no conflict in that direction" line only covered the port-bind direction. | Added an explicit note to `README.md`'s Homebrew section naming this behavior and pointing at `brew services stop saki` for an actual stop. |
| W3 | Mutation-tested: `src/commands/backend.test.ts`'s new/rewritten cases discriminate on the `healthy` boolean but never asserted `fetch` was called with the *correct* URL — pointing the probe at a wrong URL while keeping the same stub still passed all 7 tests. | Added `expect(fetchMock).toHaveBeenCalledWith(...)` to all three status sub-cases; re-ran the same mutation (wrong URL) — now fails as expected (2 tests correctly red). |
| W4 | `ensureHealthy`'s `fetch` call has no timeout — the null-state branch turned a previously-instant return into a network call bounded only by undici/OS defaults, unlike `src/daemon.ts`'s own timed probes. | Added `{ signal: AbortSignal.timeout(3_000) }` to the `fetch` call. |
| W5 | `tasks/roadmap.md`'s I4 entry still said `brew install saketek/tap/saki` — the plan itself already established `saketek` is npm-only, no such GitHub org exists (`gh api users/saketek` → 404). | Corrected to `drayanaindra/tap/saki` with the "no saketek org" note inline. |

Also confirmed by the reviewer (not fixed, pre-existing / explicitly out of scope): `e2e/daemon-lifecycle.spec.ts` only exercises the state-file-present branch, not the null-state probe path (unit tests cover it; extending e2e to a service-less-state scenario is a larger harness change, not undertaken here) — and `docs/cli-reference.md` has no `saki backend` entry at all, predating this diff.
...
- [x] `brew audit --strict --online drayanaindra/tap/saki` → CLT updated, re-ran for real: exits 0, no output (clean) — after fixing 2 real findings the first run surfaced (`version` redundant with the URL-scanned version; `assert_predicate ..., :exist?` → `assert_path_exists`, pushed as `homebrew-tap@64b52b2`)
- [x] `brew tap drayanaindra/tap && brew install drayanaindra/tap/saki && brew services start saki` → CLT updated, re-ran for real: install succeeded (`🍺 /opt/homebrew/Cellar/saki/0.1.0`); first `brew services start` reported `error 1` — root cause: a lazy CLI daemon from earlier in this session (pid 11318) still held port 8788, exactly the coexistence scenario documented in README's "Note" — `saki backend stop` freed it, `brew services restart saki` → `brew services list` showed `started` cleanly, `saki backend status` correctly reported `backend healthy (pid <real>)` via the existing state-truthy branch (confirms the reviewer's W1 finding: a same-user service gets a real state file, doesn't need the null-state probe)
- [x] `curl -s http://127.0.0.1:8788/api/health` while the service is running → `{"ok":true,"service":"saki-backend"}`
