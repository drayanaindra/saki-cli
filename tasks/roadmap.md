...
**Type:** Improvement · **Track:** Plan · **Status:** Shipped · **Owner:** unassigned · **Updated:** 2026-08-24
...
### I4 · Homebrew formula + launchd/systemd service for saki-backend
**Type:** Improvement · **Track:** Plan · **Status:** Planned · **Owner:** unassigned · **Updated:** 2026-08-24
**What:** Ship a Homebrew formula (`brew install saketek/tap/saki`) that installs the CLI + backend binary, plus a launchd `LaunchAgent` (macOS) and a systemd user unit (Linux) so `saki-backend` can run as a persistent background service (`brew services start saki`) instead of only the lazy per-command auto-start (F1's daemon lifecycle).
